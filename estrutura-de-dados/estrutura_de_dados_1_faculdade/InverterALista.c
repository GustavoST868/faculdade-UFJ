...
tipodoItem inverterLista(tipodoItem lista)
{
    ...
    tipodoItem listaAuxiliar = NULL;

    while (atual != NULL)
    {
        listaAuxiliar = atual->anterior;
        atual->anterior = atual->proximo;
        atual->proximo = listaAuxiliar;

        if (atual->anterior == NULL)
        {
            // Chegou ao novo início da lista invertida
            lista = atual;
        }

        atual = atual->anterior; // Move para o próximo nó original
    }
